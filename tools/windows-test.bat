...
if "%IsShowUsage%" == "1" (
    echo Usage   : %0  TestMode  MSR_EXE   SleepSeconds  ReplaceTo
...
echo %MSR_EXE% -p example-commands.bat -t "^msr -c (.+\s+-o\s+)" --nt "\s+-R" -o "msr -c -j $1" -PAC ^| %MSR_EXE% -x %%~dp0 -o . -a -X ^> %onlyOuputReplacedTestLog%
%MSR_EXE% -p example-commands.bat -t "^msr -c (.+\s+-o\s+)" --nt "\s+-R" -o "msr -c -j $1" -PAC | %MSR_EXE% -x %%~dp0 -o . -a -X > %onlyOuputReplacedTestLog%
