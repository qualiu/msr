...
(1) Skip/Stop reading:
    -L/-b skip rows above/not-matched-begin; -b and -Q to start/stop matching/replacing multiple blocks in each-file/pipe; with -a to output each entire block.
...
(4) Execute output line as command : If has -X (--execute-out-lines):
    ...
    only for cases which msr cannot know where to read from : files or pipe. For example, remove the -p . and -Y bellow:
...
Example-7 : Display current modified code files : Now time = 2017-07-21 23:35:17.979279 CST - China Standard Time
    for /f %a in ('msr -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2017-07-21 23:35:17" -PAC 2^>nul ') do @echo code file : %a
...
(3) Find top 100 largest old garbage log files which size >= 30MB:
    msr -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -H 100 -PIC
(4) Get command lines to delete top 100 largest old garbage log files which size >= 30MB:
    msr -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -H 100 -PAC | msr -t .+ -o "del /f /q /s \"$0\"" -PIC
(5) Delete top 100 largest old garbage log files which size >= 30MB:
    msr -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -H 100 -PAC | msr -t .+ -o "del /f /q /s \"$0\"" -X -PI
(6) Delete top 100 largest old garbage log files which size >= 30MB:
    msr -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -PAC | msr -t .+ -o "del /f /q /s \"$0\"" -H 100 -XPI
...
(1) Search in files with plain text     : msr -p %CD% -x "my plain text" -PAC
(2) Search in files with Regex pattern  : msr -p %CD% -t "my.*(capture-1).*pattern"
(3) Search & Repalce with Regex pattern : msr -p %CD% -t "my.*(capture-1).*pattern" -o "captured $1 and you want"
(4) Replace files and backup if changed : msr -p %CD% -t "my.*(capture-1).*pattern" -o "captured $1 and you want" -R -K
...
As a portable cross platform tool, msr has been running on: Windows / Linux / Cygwin / Ubuntu / CentOS / Fedora
Any good ideas please to : QQ : 94394344 , aperiodic updates and docs on https://github.com/qualiu/msr
